...
- `Expo Router (File-Based Routing)` --semantically_similar_to--> `Next.js App Router (src/app/)`  [INFERRED] [semantically similar]
  MOBILE.md → CLAUDE.md
- `AsyncStorage (Mobile Session Persistence)` --semantically_similar_to--> `src/lib/clientAuth.ts (Client JWT Auth)`  [INFERRED] [semantically similar]
  MOBILE.md → CLAUDE.md
- `NativeWind (Tailwind for React Native)` --semantically_similar_to--> `Tailwind CSS 4 (PostCSS Plugin)`  [INFERRED] [semantically similar]
  MOBILE.md → CLAUDE.md
- `Firebase Cloud Messaging (FCM) Push Notifications` --semantically_similar_to--> `VAPID Keys (Web Push)`  [INFERRED] [semantically similar]
  MOBILE.md → CLAUDE.md
- `MUSA Client App (Clientas)` --references--> `MUSA Luxury Beauty Brand Identity`  [INFERRED]
  MOBILE.md → apps/client/assets/icon.png
